...
public class Both_Are_IncorrectTC4 extends Redbus_TestCase_Excel {
	...
	public void openbrowser() throws RowsExceededException, WriteException, IOException {
		...
		driver.get(baseUrl);
		Both_Are_IncorrectTC4.screen("screenshot");
		...
		if (Main_Page_Title.equalsIgnoreCase("Book Bus Travels, AC Volvo Bus, rPool & Bus Hire - redBus India")) {
			System.out.println("The BothAreIncorrect PASS");
		} else {
			System.out.println("The BothAreIncorrect FAIL");
		}
	}

	@Test(priority = 1, description = "Testcase One")
	public void BothAreIncorrect() throws InterruptedException, IOException {
		...
		while (itr.hasNext()) {
			...
			if (!mainWindow.equalsIgnoreCase(childWindow)) {
				...
				String username = writablesh.getCell(2, 4).getContents();
				...
				if (StoreErrorMsg.equalsIgnoreCase("Enter a valid email or phone number")) {
					System.out.println("The BothAreIncorrect Fail due to Incorrect Username");
					driver.close();
	...
	public static void screen(String screenshotName) throws IOException {
		...
		FileHandler.copy(screenshot, new File("C:\\Users\\Senthilnathan\\workspace\\Automation_Project_Redbus\\"
				+ "Screenshots_For_ALL\\ScreenshotTC4_Both_Are_Incorrect\\" + screenshotNameFormat + ".png"));
	}
}
